...
async function processDirectory(url, depth = 0) {
	...
	for (let elem of documents) {
		...
		if (documentExt === "htm") {
			downloadUrl = `https://www.bclaws.gov.bc.ca/civix/document/id/complete/statreg/${documentId}_multi/xml`;
		} else if (documentExt === "xml") {
			downloadUrl = `https://www.bclaws.gov.bc.ca/civix/document/id/complete/statreg/${documentId}/xml`;
		}
		...
		if (downloadUrl) {
			...
			const filename = path.join(
				__dirname,
				"downloads",
				`${sanitizedTitle}.${documentExt === "htm" ? "multi.xml" : "xml"}`
			);
